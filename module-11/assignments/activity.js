  ...
  const html = source.reduce((acc, { name, id, icons: [icon], playlists: { playlistData } }) => {
    const playlistsList = playlistData.map(
      ({ name, external_urls: { spotify }, images: [image], tracks }) => {
        ...
        return `
            <li><a href="${spotify}" alt="${name}" target="_blank"><img src="${image.url}" width="180" height="180"/></a>
               <div> ${tracksList}</div>
            </li>`
      }).join(``);
    ...
    if (playlistData) {
      const html = `
        <article class="genre-card">
            <div><h2>${name}</h2><img src="${icon.url}" width="${icon.width}" height="${icon.height}" alt="${name}"/></div>
            <div><ol>${playlistsList}</ol></div>
        </article>`;
      return acc + html;
    }
  }, ``);
