...
const active = data.filter((accounts) => accounts.isActive);
console.log(active);

const {balance} = data.reduce((acc,balance) => {
    return acc > balance? acc : balance;
})
console.log(balance);

const friends = data.map((person) => {
    return person.friends.map((each) => each.name);
})
console.log(friends);

const accountsName = data.map(({ name }) => name);
const listName = accountsName.join(', ');
