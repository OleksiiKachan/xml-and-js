const people = [
 ...
 //Solution 1
  const AreActive = people.filter((people)=>people.isActive);
  console.log(AreActive);
  
  //Solution 2
  const highestBalance = people.reduce((acc, cur) => {
  ...
  const onlyfriends = people.map((people)=>{
    people.friends.name;
  });
  ...
  const getFriends = (people) => {
    return people.reduce((mystr, person) => {
      return mystr.concat(person.friends.map((friend) => friend.name));
    }, []);
  };
  
  console.log(getFriends(people));
