...
const getActiveAccount = () => {
    return json2.filter(e => e.isActive)
}
console.log(getActiveAccount());

const getHigestBalance = () => {
    const balance = json2.reduce((prev, crr) => prev = parseInt(prev) > parseInt(crr.balance) ? prev : crr.balance, 0);
    return balance.toString();
}
console.log(getHigestBalance());

const getAllFriendsName = () => {
    return json2.reduce((prev, crr) => prev = [...prev, ...(crr.friends.map(e => e.name))] , [])
}
console.log(getAllFriendsName());

const getAccountHolderNames = () => {
    const nameArr = json2.map(e => e.name);
    return nameArr.join(', ')
}
