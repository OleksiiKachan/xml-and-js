...
const people = data.filter((person) => person);

//console.log(people);

const id = people.map((people) => (people.id));
const firstName = people.map((people) => (people.first_name));
const lastName = people.map((people) => (people.last_name));
const email = people.map((people) => (people.email));
const gender = people.map((people) => (people.gender));
const ipAddress = people.map((people) => (people.ip_address));

const person = people.map((id, firstName, lastName, email, gender, ipAddress) => person);
