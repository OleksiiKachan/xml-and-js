const people = [
  ...
  const people_filtered = people.map(({id, first_name, last_name, gender, ip_address}) =>{
    const firstName = first_name; // you can do it this way too

    return{
        id,
        firstName,
        lastName:last_name, // this and the above is the same, with two different ways
        gender,
  ...
  //console.log(people_filtered);

  const parsed = people_filtered.filter((item) => item.id >= 5);
