...
const getByGenre = async (genre) => {
  const response = await fetch(
    `http://www.omdbapi.com/?apikey=25bcda46&s=${genre}`
  );
  const data = await response.json();
...
const onSubmit = async (event) => {
  ...
  if (search !== "") {
    ...
    // when search text is empty, pumpt warning
  } else {
    display_div.innerHTML = `<div class="error">Genre, Title, or IMDb ID is required.</div>`;
  }
};
