  ...
  console.log(getActiveAccounts(accounts));


// Function to get the highest balance.

const getHighestBalance = accounts => {
    const highestBalance = accounts.reduce((acc, account) => {
      if (account.balance > acc) {
        acc = account.balance;
      }
      return acc;
    }, 0);
  ...
  console.log(getHighestBalance(accounts));
  
  
  // Function to get array of friends of all accounts. Function should return array names
  const getAllFriends = accounts => {
    const allFriends = accounts.reduce
    ((acc, account) => {
...
// Function to get string of account holders names
  function getAllNames(accounts) {
    return accounts.map(account => account.name).join(', ');
  }
    console.log(getAllNames(accounts));